...
void SDMLandmarkDetector::draw(cv::Mat &img, const cv::Scalar &color, bool show, std::string &img_save_path)
{
...
void SeetaLandmarkDetector::draw(cv::Mat &img, const cv::Scalar &color, bool show, std::string &img_save_path)
{
...
void DlibLandmarkDetector::draw(cv::Mat &img, const cv::Scalar &color, bool show, std::string &save_img_path)
{
	int face_num = m_face_boxes.size();
	for (int i = 0; i < face_num; i++)
	{
...
void dlib_config_emotion_mark(cv::Mat &img, dlib::full_object_detection &shape, landmark_t *mark)
{
	...
	float Lmax = (Lx > Ly ? Lx : Ly) * 1.2;
	int delta = int(Lmax / 2);
	int start_x = mark->nose.x - delta;
	int start_y = mark->nose.y - delta;
	int end_x = mark->nose.x + delta;
	int end_y = mark->nose.y + delta;
	#endif
...
{
	mark->left_brow_outer = cv::Point(shape.part(17).x(), shape.part(17).y());
	mark->left_brow_inner = cv::Point(shape.part(21).x(), shape.part(21).y());

	mark->right_brow_inner = cv::Point(shape.part(22).x(), shape.part(22).y());
	mark->right_brow_outer = cv::Point(shape.part(26).x(), shape.part(26).y());

	mark->left_eye_outer = cv::Point(shape.part(36).x(), shape.part(36).y());
	mark->left_eye_inner = cv::Point(shape.part(39).x(), shape.part(39).y());
	mark->left_eye = cv::Point((mark->left_eye_inner.x + mark->left_eye_outer.x) / 2,
								(shape.part(37).y() + shape.part(41).y()) / 2);

	mark->right_eye_inner = cv::Point(shape.part(42).x(), shape.part(42).y());
	mark->right_eye_outer = cv::Point(shape.part(45).x(), shape.part(45).y());
	mark->right_eye = cv::Point((mark->right_eye_inner.x + mark->right_eye_outer.x) / 2,
								(shape.part(43).y() + shape.part(47).y()) / 2);

	mark->nose = cv::Point(shape.part(30).x(), shape.part(30).y());
	mark->nose_left_corner = cv::Point(shape.part(31).x(), shape.part(31).y());
	mark->nose_right_corner = cv::Point(shape.part(35).x(), shape.part(35).y());

	mark->mouth_left = cv::Point(shape.part(48).x(), shape.part(48).y());
    mark->mouth_right = cv::Point(shape.part(54).x(), shape.part(54).y());
    mark->mouth_bottom = cv::Point(shape.part(57).x(), shape.part(57).y());

	mark->chin = cv::Point(shape.part(8).x(), shape.part(8).y());

	mark->center = cv::Point(shape.part(29).x(), shape.part(29).y());
...
int DlibLandmarkDetector::detect(cv::Mat &img, std::vector<seeta::FaceInfo> *faces, vector<landmark_t> *mark)
{
	...
	std::vector<dlib::rectangle> dlib_faces = m_detector(cimg);
	...
	for (int i = 0; i < faces->size(); i++)
	{
		...
		dlib::rectangle face;
		#if 1
		if (dlib_faces.size() > 0 && i < dlib_faces.size())
		{
			face = dlib_faces[i];
		}
		else
		{
			face = dlib::rectangle(faces->at(i).bbox.x, faces->at(i).bbox.y,
					  			faces->at(i).bbox.x + faces->at(i).bbox.width,
					  			faces->at(i).bbox.y + faces->at(i).bbox.height);
		}
		#else
		...
		mark_item.face_box = cv::Rect(faces->at(i).bbox.x, faces->at(i).bbox.y,
								    faces->at(i).bbox.width, faces->at(i).bbox.height);
		dlib_landmarks_to_mark(gray, shape, &mark_item);
...
void Detector::landmark_draw(cv::Mat &img, const cv::Scalar &color, bool show, std::string &img_save_path)
{
