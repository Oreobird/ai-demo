...
int main(int argc, char** argv)
{
    ...
    printf("\nReading in %s...\n\n",filename.c_str());
    const double train_test_split_ratio = 0.8;
    ...
    rtrees->setTermCriteria(TermCriteria(TermCriteria::MAX_ITER, 200, 0.01));
    ...
    rtrees->save("score_rtree.xml");

    Ptr<RTrees> haha = Algorithm::load<RTrees>("score_rtree.xml");
    cv::Mat param = (Mat_<float>(1, 8) << 0.215,0.99,19.6,24.3,4.063,0.807,0.9,0.98);
    float r = haha->predict(param);
