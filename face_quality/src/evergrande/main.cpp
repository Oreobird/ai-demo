...
#include "fq_req_tcp_event_handler.h"
...
{
    fprintf(stderr,
        "Usage: ./fq_srv -s cfg_file -m model_dir -l log_dir\n\n"
        "  -s  config file\n\n"
        "  -m  model dir\n\n"
        "  -l  log_dir\n\n"
        "  -h  help info\n\n"
...
int main(int argc, char * argv[])
{
	...
	printf("--- prepare to init detector mgt ---\n");
	...
	g_detector_mgt = new Detector_Mgt;

	nRet = g_detector_mgt->init(model_dir);
	if (nRet != 0)
	{
		printf("init detector mgt failed, ret:%d\n", nRet);
		return 0;
	}

	printf("===== complete to init detector mgt =====\n");
