...
int EvgJson::gen_exception_json(int face_num, std::string &img_data, std::string &json_str)
{
	Json::Value root;
	Json::FastWriter fw;
	...
	else if (face_num > 1)
	{
		...
		root["img_data"] = Json::Value(img_data);
	}

	json_str = fw.write(root);
	return 0;
}

int EvgJson::gen_face_json(landmark_t *mark, Quality *qof, std::string &img_data, std::string &json_str)
{
    Json::Value root;
    Json::FastWriter fw;
    ...
    root["pass"] = Json::Value((qof->is_pass() && score >= QUALITY_THRESHOLD) ? true : false);
