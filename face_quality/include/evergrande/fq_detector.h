#ifndef __FQ_DETECTOR_H__
#define __FQ_DETECTOR_H__
...
{
    cv::Point left_brow_outer;
    cv::Point left_brow_inner;
    cv::Point right_brow_outer;
    cv::Point right_brow_inner;
    cv::Point left_eye_inner;
    cv::Point right_eye_inner;
    cv::Point nose_left_corner;
    cv::Point nose_right_corner;
    cv::Point mouth_bottom;
...
class ILandmarkDetector
{
	public:
		...
		virtual void draw(cv::Mat &img, const cv::Scalar &color, bool show, std::string &img_save_path) = 0;
};
...
class SDMLandmarkDetector:public ILandmarkDetector
{
	public:
		...
		void draw(cv::Mat &img, const cv::Scalar &color, bool show, std::string &img_save_path);
...
class Detector
{
	public:
		...
		void landmark_draw(cv::Mat &img, const cv::Scalar &color, bool show, std::string &img_save_path);
