...
int recv_from_server(evg_msg_t *msg)
{
	if (g_client.fd > 0)
	{
		...
		if (buf_len > 0)
		{
			...
			cv::Mat img_result = mat_codecs.base64_to_mat(img_result_str);
			...
			#else
			nRet = XProtocol::get_specific_params(msg->buf, "video_path", img_result_str, errInfo);
			if (!img_result_str.empty())
			{
				std::cout << "video_path:" << img_result_str << std::endl;
			}

			nRet = XProtocol::get_specific_params(msg->buf, "progress", img_result_str, errInfo);
			if (!img_result_str.empty())
			{
				std::cout << "progress:" << img_result_str << std::endl;
			}
			#endif
...
int main(int argc, char** argv)
{
    ...
    ret = g_client.connect_srv("127.0.0.1", 6676);
    if (ret < 0)
    {
	...
	MatCodecs mat_codecs;
	std::string data_str = mat_codecs.mat_to_base64(img);
    dbg("data_str len:%d\n", (int)(data_str.length()));
	#endif
    ...
    #if IMG_TEST
    req << "{\"img_data\":\"" << data_str
    #else
	req << "{\"video_path\":\"" << img_path
	#endif
		<< "\",\"line_points\":[";
